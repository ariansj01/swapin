...
function render_categories_strip(?int $active = null): void {
    ...
    foreach ($cats as $c) {
        ...
        echo "<a href='{APP_URL}/category/{$c['slug']}' class='cat-pill{$cls}'><div class='cat-pill__icon'><i class='{$c['icon']}'></i></div><span class='cat-pill__label'>" . category_label($c['slug'], $c['name']) . "</span></a>";
    }
    echo '</div>';
}
