...
$page = DB::fetch('SELECT * FROM content_pages WHERE id = ?', [$id]);
...
<input class="form-control" name="title" value="<?= h($page['title']) ?>" required>
</div>

<div class="form-group">
<label>Slug</label>
<input class="form-control" name="slug" value="<?= h($page['slug']) ?>" required>
</div>

<div class="form-group">
<label>محتوا</label>
<textarea class="form-control" name="content" rows="12"><?= h($page['content']) ?></textarea>
</div>

<div class="form-group">
<label>تصویر شاخص / آدرس تصویر</label>
<input class="form-control" name="featured_image" value="<?= h($page['featured_image'] ?? '') ?>">
</div>

<div class="form-group">
<label>Meta Title</label>
<input class="form-control" name="meta_title" value="<?= h($page['meta_title']) ?>">
</div>

<div class="form-group">
<label>Meta Description</label>
<textarea class="form-control" name="meta_description" rows="4"><?= h($page['meta_description']) ?></textarea>
</div>
...
<textarea class="form-control" name="internal_links" rows="4" placeholder="هر خط: عنوان | لینک"><?= h($page['internal_links'] ?? '') ?></textarea>
</div>
...
<option value="index" <?= $page['index_status'] === 'index' ? 'selected' : '' ?>>Index</option>
<option value="noindex" <?= $page['index_status'] === 'noindex' ? 'selected' : '' ?>>NoIndex</option>
</select>
</div>

<label><input type="checkbox" name="show_in_nav" value="1" <?= (int)$page['show_in_nav'] === 1 ? 'checked' : '' ?>> نمایش در Navbar</label>
<br>
<label><input type="checkbox" name="show_in_footer" value="1" <?= (int)$page['show_in_footer'] === 1 ? 'checked' : '' ?>> نمایش در Footer</label>
...
<div class="faq-item">
<input class="form-control" name="faq[<?= (int)$i ?>][question]" value="<?= h($faq['question'] ?? '') ?>" placeholder="سوال FAQ">
<br>
<textarea class="form-control" name="faq[<?= (int)$i ?>][answer]" placeholder="پاسخ FAQ"><?= h($faq['answer'] ?? '') ?></textarea>
</div>
<?php endforeach; ?>
</div>
